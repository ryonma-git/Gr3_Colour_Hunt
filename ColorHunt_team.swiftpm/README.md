...
ORANGE を S 0.65以上に限り、BROWN を S 0.38〜0.70・V 0.18〜0.85 とすることで
分けています（S 0.65〜0.70 のくすんだオレンジは両方に入ります）。
木の机・床・段ボール・茶色い筆箱が通ることを優先しています。
...
### ★ 児童が「許可しない」を押してしまったとき

**設定に「Color Hunt」という項目は出ません。** Swift Playgrounds から実行している
アプリのカメラ許可は、**Swift Playgrounds 自体**に紐づくためです。

上から順に試してください。

1. **設定 →「Swift Playgrounds」→「カメラ」をオン**
2. 見つからない場合: **設定 → プライバシーとセキュリティ → カメラ** の一覧で
   `Swift Playgrounds` をオン
3. それでも出てこない場合: **設定 → 一般 → 転送またはiPadをリセット → リセット →
   位置情報とプライバシーをリセット**
   （データは消えません。全アプリの許可が「まだ聞いていない」状態に戻ります）

**これを起こさないために**、アプリ側で先に予告画面を出すようにしてあります。

```
        📷
   つぎに「カメラを つかっても いい？」と きかれます
        「OK」を おしてね
          [ すすむ ]
```

「すすむ」を押すまで iOS のダイアログは出ません。授業では
**「つぎに出る画面は かならず OK を押します」** と一声かけてから START させてください。
...
8色の値は次のとおりです（Hue は色相・S は鮮やかさ・V は明るさ）。
2026-09-14 の授業で PURPLE・YELLOW などが反応しにくかったため、ORANGE 以外をゆるめました（profileVersion 2）。

| 色 | Hue | S | V | ねらい |
|---|---|---|---|---|
| RED | 340–360, 0–14 | 0.45+ | 0.20+ | S を高めにして肌とピンクを除く |
| BROWN | 10–45 | 0.38–**0.70** | 0.18–0.85 | ORANGE と色相が同じなので S で分ける（TEAM 8 用） |
| ORANGE | 16–44 | **0.65+** | 0.50+ | S を高めにして木の机・肌を除く（変更なし） |
| YELLOW | 40–75 | 0.28+ | 0.45+ | 山吹色・白っぽく写った黄色・影の黄色も通す |
| GREEN | 70–175 | 0.20+ | 0.15+ | 黄緑から青緑まで。濃い緑の黒板も通す |
| BLUE | 180–250 | 0.22+ | 0.18+ | ターコイズ・うすい水色から紺色まで |
...
PINK は2つの範囲の合わせ技です。赤と色相が重なるところ（①）は S の上限で分け、
赤の無い色相（②）では S の上限をなくしています（`ColorProfile.extraRegions`）。

境界の近くでは、2つの色に入る物があります（山吹色は YELLOW と ORANGE、赤むらさきは PURPLE と PINK など）。
アプリは「いまさがしている色」に入っているかだけを確かめるので、誤作動にはなりません。
どちらの色とも言える物はどちらでも見つかる、というだけです（色を決めるのは子ども、という方針どおり）。
...
### 検証済みの誤判定チェック（2026-09-14・シミュレーション）

JIS慣用色名の色見本と、教室にありそうな物の想定色を、露出4段階 × 照明の色味3種（12通り）に
揺らしてカメラ越しの見え方を近似し、判定を数えました。**授業での実測ではありません。**

| | 変更前 | 変更後 |
|---|---|---|
| その色の物が通る割合 | 74.6% | **98.2%** |
| 白い壁・肌・机・黒い服などが誤って通る | 4 / 1848 | 6 / 1848 |

変更後に残る誤判定は、RED をさがすときの赤っぽい焦げ茶（変更前から同じ）と、
PURPLE をさがすときの黒い服・YELLOW をさがすときの白木の棚（それぞれ12通り中1通り）だけです。
肌（明るめ・手のひら・日焼けの3種）は、BROWN（既知の制約）を除き、どの色にも通りませんでした。
Web 版と Swift 版の判定が一致することも、HSV を細かく区切った全点（1色あたり約734万点）で確認しています。
...
- FOUND（緑）になったあと、対象の色から `HuntTuning.foundReleaseDuration` 秒（既定 0.8秒。
  みつける 0.5秒より少し長め）つづけて外れると、白にもどってまた探し始められます。
  カメラを別のものに向けかえるだけで、何度でも探し直せます
- 0.8秒より長くカメラが外れると、シャッターを押す前に緑が消えます
  （2026-09-14 の授業で 0.5秒では早すぎたため、0.8秒にしました）。
  まだ早いようなら `foundReleaseDuration` を 1.0 くらいに伸ばしてください
